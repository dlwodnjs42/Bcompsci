...
class Game {
    ...
    /** Record of the game. */
    private static ArrayList<Game> gamerecorder = new ArrayList<Game>();
    ...
    /** Undo's a move in the game. */
    void undo() {
        if (gamerecorder.isEmpty()) {
            return;
        } else {
            copyFrom(gamerecorder.get(gamerecorder.size() - 1));
            gamerecorder.remove(gamerecorder.size() - 1);
        }
    }
