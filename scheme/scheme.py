...
def eval_all(expressions, env):
    ...
    #Pair(2,Pair(3, nil))
    #recurse to scheme_eval for the first value of expressions (integer) 
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        symbol = target.first
        env.define(symbol, do_lambda_form(Pair(target.second, expressions.second), env))
        return symbol
    else:
...
def do_if_form(expressions, env):
    ...
    if scheme_false(scheme_eval(expressions.first, env)):
        if expressions.second.second is nil:
            return okay #if there is no False value at all return okay
        else:
            return scheme_eval(expressions.second.second.first, env, True)
    elif scheme_true((expressions.first, env)):
        return scheme_eval(expressions.second.first, env, True) 
    else:
        return okay # if True value is a non-evaluated value
...
def do_and_form(expressions, env):
    ...
    while expressions.second != nil:
        ...
        if scheme_false(evaluated_expression):
...
def do_or_form(expressions, env):
    ...
    while expressions.second != nil:
        evaluated_expression = scheme_eval(expressions.first, env)
        if scheme_true(evaluated_expression):
            return evaluated_expression
        else:
            expressions = expressions.second
    return scheme_eval(expressions.first, env, True)
...
def check_formals(formals):
    ...
    checked_formals = []
    while formals != nil:
        if formals.first in checked_formals:
            raise SchemeError 
        elif scheme_symbolp(formals.first) == True:
            checked_formals.append(formals.first)
            formals= formals.second
        else:
            raise SchemeError
