...
def group_by_centroid(restaurants, centroids):
    ...
    """
    restaurant_by_centroid = []
    for x in restaurants:
        restaurant_by_centroid.append([find_closest(restaurant_location(x), centroids), x])
    return group_by_first(restaurant_by_centroid)
...
def find_centroid(restaurants):
    ...
    average_centroid_y = []
    for x in restaurants:
        average_centroid_x.append(restaurant_location(x)[0])
        average_centroid_y.append(restaurant_location(x)[1])
    return [mean(average_centroid_x), mean(average_centroid_y)]
    #made two blank lists to seperate all the x values of the locations and y values of the locations.
    #took the mean of the two lists containing all the x values and y values to find the centroid of the location of the restaurants.
...
def k_means(restaurants, k, max_updates=100):
    ...
    while old_centroids != centroids and n < max_updates:
        ...
        restaurants_clusters = group_by_centroid(restaurants, centroids)
        centroids = [find_centroid(restaurants) for restaurants in restaurants_clusters]
        n += 1
    return centroids
...
def find_predictor(user, restaurants, feature_fn):
    ...
    """
    restaurant_copy = restaurants
    reviews_by_user = {review_restaurant_name(review): review_rating(review)
                       for review in user_reviews(user).values()}

    xs = [feature_fn(r) for r in restaurant_copy]
    ys = [reviews_by_user[restaurant_name(r)] for r in restaurant_copy]
    ...
    S_xx = sum_squares(xs,xs)
    S_yy = sum_squares(ys,ys)
...
def best_predictor(user, restaurants, feature_fns):
    ...
    for fn in feature_fns:
        temp_predictor, temp_r2 = find_predictor(user, reviewed, fn)
        if goal_r2 < temp_r2:
...
def rate_all(user, restaurants, feature_functions):
    ...
    restaurant_copy = restaurants.copy()

    predictor = best_predictor(user, RESTAURANTS, feature_functions)
    for r in restaurant_copy:

        if r in user_reviewed_restaurants(user,restaurant_copy):

            restaurant_copy[r] = user_rating(user, r)
        else:
            restaurant_copy[r] = predictor(restaurant_copy[r])
...
def search(query, restaurants):
    ...
    return [r for r in restaurants for x in restaurant_categories(r) if x == query]
